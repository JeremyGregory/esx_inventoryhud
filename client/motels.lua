...
ESX.TriggerServerCallback("utk_motels:getRoomInventory", function(inventory)
...
TriggerServerEvent("utk_motels:putItem", ESX.GetPlayerData().identifier, data.item.type, data.item.name, count)
end
...
if type(data.number) == "number" and math.floor(data.number) == data.number then
TriggerServerEvent("utk_motels:getItem", ESX.GetPlayerData().identifier, data.item.type, data.item.name, tonumber(data.number))
end
