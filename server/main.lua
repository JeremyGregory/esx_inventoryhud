	...
	function(from, target, type, itemName, itemCount)
		...
		if type == "item_standard" then
			...
			if itemCount > 0 and sourceItem.count >= itemCount then
				...
				--if targetItem.weight ~= -1 and (targetItem.count + itemCount) > targetItem.weight then
					...
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = _U('item_removed') .. itemName})
					targetXPlayer.addInventoryItem(itemName, itemCount)
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = _U('item_added') .. itemName})
				else
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = _U('player_inv_no_space') .. itemName})
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = _U('player_inv_no_space') .. itemName})
				end
			end
		elseif type == "item_money" then
			if itemCount > 0 and sourceXPlayer.getMoney() >= itemCount then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = _U('money_removed') .. itemCount})
				targetXPlayer.addMoney(itemCount)
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = _U('money_added') .. itemCount})
			end
		elseif type == "item_account" then
			if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
				sourceXPlayer.removeAccountMoney(itemName, itemCount)
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = _U('money_removed') .. itemCount .. itemName})
				targetXPlayer.addAccountMoney(itemName, itemCount)
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = _U('money_added') .. itemCount .. itemName})
			end
		elseif type == "item_weapon" then
			if not targetXPlayer.hasWeapon(itemName) then
				sourceXPlayer.removeWeapon(itemName)
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = _U('weapon_removed') .. itemName})
				targetXPlayer.addWeapon(itemName, itemCount)
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = _U('waepon_added') .. itemName})
			end
		end
	end
)
